...
import { SPECIES_MODEL_URL as MODEL_URL } from "./speciesModels";

// MODEL_URL (key -> GLB url) now lives in ./speciesModels — a plain data
// module with no react-three-fiber/drei deps, so scripts/render-thumbnails.ts
// can import the exact same source of truth Node-side without dragging in
// client-only rendering libraries. Deliberately NOT preloaded here — with
// 30+ species now in the roster, eagerly fetching all of them the moment any
// avatar renders would waste bandwidth on animals a given student may never
// own; useGLTF's Suspense integration fetches one on demand exactly when
// it's equipped.
