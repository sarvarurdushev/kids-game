...
function buildRound(roundIndex: number, usedWords: Set<string>): RoundData {
  ...
  const blankIndex = Math.floor(Math.random() * target.word.length);
  const correctLetter = target.word[blankIndex];
  const decoys = shuffle(ALPHABET.filter((l) => l !== correctLetter)).slice(0, 3);
  return { target, blankIndex, correctLetter, choices: shuffle([correctLetter, ...decoys]) };
}
...
export function MissingLetter({ equippedKeys }: { equippedKeys: AvatarEquippedKeys }) {
  ...
  function pick(letter: string) {
    ...
    const correct = letter === roundRef.current.correctLetter;
  ...
  async function endGame(finalCorrect: number, roundsPlayed: number) {
    ...
    playGameOver();
    try {
      const res = await fetch("/api/games/missing_letter/complete", {
        method: "POST",
  ...
  const displayWord = round.target.word
    .split("")
    .map((c, i) => (i === round.blankIndex ? "_" : c))
    .join("");
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="flex flex-col items-center gap-2 rounded-3xl bg-white/80 py-4 shadow-sm">
        ...
        <p className="font-display text-2xl font-bold uppercase tracking-widest text-ink">{displayWord}</p>
      </div>

      <div className="grid grid-cols-4 gap-2">
        {round.choices.map((letter) => {
          const isPicked = pickedLetter === letter;
          const revealCorrect = phase === "result" && letter === round.correctLetter;
          const tone =
            phase === "result"
              ? revealCorrect
          ...
          return (
            <button
              ...
              onClick={() => pick(letter)}
              className={`font-display rounded-2xl border-2 py-4 text-xl font-bold uppercase transition-all ${tone}`}
            >
              {letter}
            </button>
          );
