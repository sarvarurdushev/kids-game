...
function buildRound(roundIndex: number, usedWords: Set<string>): RoundData {
  ...
  if (isTrue) return { emoji: target, shownWord: target.word, isTrue: true };
  const [decoy] = randomWords(1, pool, target);
  return { emoji: target, shownWord: decoy.word, isTrue: false };
}

// Same refs-for-synchronous-state pattern as EmojiQuiz/WordCatch.
export function TrueOrFalse({ equippedKeys }: { equippedKeys: AvatarEquippedKeys }) {
  ...
  function startRound(index: number) {
    ...
    usedWordsRef.current.add(data.emoji.word);
    roundRef.current = data;
    setRound(data);
    setPhase("playing");
  }

  function pick(answerTrue: boolean) {
    if (phase !== "playing" || !roundRef.current) return;
    const correct = answerTrue === roundRef.current.isTrue;
    setPickedTrue(answerTrue);
  ...
  async function endGame(finalCorrect: number, roundsPlayed: number) {
    ...
    playGameOver();
    try {
      const res = await fetch("/api/games/true_or_false/complete", {
        method: "POST",
  ...
  if (phase === "ready") {
    return (
      <div className="flex flex-col items-center gap-5 text-center">
        ...
        <div>
          <h1 className="font-display text-2xl font-bold text-gold-dark">{t("game.trueOrFalse.name")}</h1>
          <p className="mt-1 text-sm text-ink/60">{t("game.trueOrFalse.instructions")}</p>
        </div>
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="flex flex-col items-center gap-2 rounded-3xl bg-white/80 py-4 shadow-sm">
        ...
        <div className="text-6xl">{round.emoji.emoji}</div>
        <p className="font-display text-2xl font-bold capitalize text-ink">{round.shownWord}</p>
      </div>

      <div className="grid grid-cols-2 gap-3">
        {[
          { key: "yes", labelKey: "game.trueOrFalse.yes" as const, value: true },
          { key: "no", labelKey: "game.trueOrFalse.no" as const, value: false },
        ].map((option) => {
          const isPicked = pickedTrue === option.value;
          const revealCorrect = phase === "result" && option.value === round.isTrue;
          const tone =
            phase === "result"
              ? revealCorrect
          ...
          return (
            <button
              ...
              className={`font-display rounded-2xl border-2 py-5 text-2xl font-bold transition-all ${tone}`}
            >
              {t(option.labelKey)}
            </button>
          );
      ...
      <AnimatePresence>
        {phase === "result" && (
          ...
          >
            {lastCorrect
              ? t("games.greatJob")
              : `${t("games.notQuite")} ${t("gameResult.wasActually", { word: round.emoji.word })}`}
          </motion.p>
        )}
      </AnimatePresence>
    </div>
