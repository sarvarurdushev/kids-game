...
export function PicturePick({ equippedKeys }: { equippedKeys: AvatarEquippedKeys }) {
  ...
  function pick(emoji: string) {
    ...
    const correct = emoji === roundRef.current.target.emoji;
  ...
  async function endGame(finalCorrect: number, roundsPlayed: number) {
    ...
    playGameOver();
    try {
      const res = await fetch("/api/games/picture_pick/complete", {
        method: "POST",
  ...
  if (phase === "ready") {
    return (
      <div className="flex flex-col items-center gap-5 text-center">
        ...
        <div>
          <h1 className="font-display text-2xl font-bold text-gold-dark">{t("game.picturePick.name")}</h1>
          <p className="mt-1 text-sm text-ink/60">{t("game.picturePick.instructions")}</p>
        </div>
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="flex flex-col items-center gap-2 rounded-3xl bg-white/80 py-4 shadow-sm">
        ...
        <p className="font-display text-2xl font-bold capitalize text-ink">{round.target.word}</p>
        <p className="text-sm font-semibold text-ink/60">{t("game.picturePick.whichPicture")}</p>
      </div>

      <div className="grid grid-cols-2 gap-3">
        {round.choices.map((choice) => {
          const isPicked = pickedEmoji === choice.emoji;
          const revealCorrect = phase === "result" && choice.emoji === round.target.emoji;
          const tone =
            phase === "result"
              ? revealCorrect
      ...
      <AnimatePresence>
        {phase === "result" && (
          ...
          >
            {lastCorrect ? t("games.greatJob") : t("games.notQuite")}
          </motion.p>
        )}
      </AnimatePresence>
    </div>
