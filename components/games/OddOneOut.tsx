...
function buildRound(roundIndex: number): RoundData {
  ...
  const mainPool = pool.filter((w) => w.category === mainCategory);
  const otherPool = pool.filter((w) => w.category === otherCategory);
  const three = shuffle(mainPool).slice(0, 3);
  const oddOne = shuffle(otherPool)[0];

  return { choices: shuffle([...three, oddOne]), oddOne };
}

// Same refs-for-synchronous-state pattern as EmojiQuiz/WordCatch.
export function OddOneOut({ equippedKeys }: { equippedKeys: AvatarEquippedKeys }) {
  ...
  function pick(word: string) {
    ...
    const correct = word === roundRef.current.oddOne.word;
    setPickedWord(word);
  ...
  async function endGame(finalCorrect: number, roundsPlayed: number) {
    ...
    playGameOver();
    try {
      const res = await fetch("/api/games/odd_one_out/complete", {
        method: "POST",
  ...
  if (phase === "ready") {
    return (
      <div className="flex flex-col items-center gap-5 text-center">
        ...
        <div>
          <h1 className="font-display text-2xl font-bold text-gold-dark">{t("game.oddOneOut.name")}</h1>
          <p className="mt-1 text-sm text-ink/60">{t("game.oddOneOut.instructions")}</p>
        </div>
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="flex flex-col items-center gap-1 rounded-3xl bg-white/80 py-3 shadow-sm">
        ...
        <p className="text-sm font-semibold text-ink/60">{t("game.oddOneOut.whichOne")}</p>
      </div>

      <div className="grid grid-cols-2 gap-3">
        {round.choices.map((choice) => {
          const isPicked = pickedWord === choice.word;
          const revealCorrect = phase === "result" && choice.word === round.oddOne.word;
          const tone =
            phase === "result"
              ? revealCorrect
              ...
              : "border-ink/10 bg-white/90 hover:border-gold active:scale-95";
          return (
            <button
              key={choice.word}
              type="button"
              ...
              className={`rounded-2xl border-2 py-5 text-5xl transition-all ${tone}`}
            >
              {choice.emoji}
            </button>
          );
      ...
      <AnimatePresence>
        {phase === "result" && (
          ...
          >
            {lastCorrect
              ? t("games.greatJob")
              : t("game.oddOneOut.notQuiteDidntBelong", { word: round.oddOne.word })}
          </motion.p>
        )}
      </AnimatePresence>
    </div>
